...
/**
 * useSettings 返回类型接口定义
 */
interface UseSettingsReturn {
  settings: Ref<SettingItem[]>
  handleSettingChange: (id: string, value: boolean | string) => void
...
export function useSettings(): UseSettingsReturn {
   ...
   * 处理设置项变更
   */
  const handleSettingChange = (id: string, value: boolean | string): void => {
    ...
    if (setting) {
      ...
      // 特殊处理深色模式
      if (id === 'darkMode') {
        document.documentElement.classList.toggle('dark-mode', value as boolean)
      }
    }
  }
  ...
  const handleClearCache = async (): Promise<void> => {
    try {
      await ElMessageBox.confirm(
        '确定要清除所有缓存数据吗？这将删除本地存储的对话记录和分析结果。',
        '确认清除',
        {
